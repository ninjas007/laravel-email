...
@section('content-app')
    <form action="{{ route('messages.update', encodeId($message->id)) }}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row mb-2">
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-title pt-2">Edit Pesan</h3>
                    </div>
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <input type="text" name="name" class="form-control" required value="{{ $message->name }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <input type="text" name="subject" class="form-control" required value="{{ $message->subject }}">
                            ...
                            <div class="col-sm-6">
                                <select name="list_id" id="list_id" class="select2 form-control" required>
                                    @foreach ($lists as $list)
                                        <option value="{{ $list->id }}" {{ $message->list_id == $list->id ? 'selected' : '' }}>{{ $list->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-6">
                                <select name="template_id" id="templateId" class="select2-single form-control" required>
                                    @foreach ($templates as $template)
                                        <option value="{{ $template->id }}" {{ $message->template_id == $template->id ? 'selected' : '' }}>{{ $template->name }}</option>
                                    @endforeach
                                </select>
